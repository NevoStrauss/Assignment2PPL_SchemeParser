...
export const parseClassExp = (fields: Sexp, methods: Sexp): Result<ClassExp> => {
    ...
    const bindingsResult = bind(valsResult, (vals: CExp[]) => makeOk(zipWith(makeBinding, vars, vals)));
    return isArray(fields) && allT(isString,fields) && isOk(bindingsResult) &&!isEmpty(fields) ?
            makeOk(makeClassExp(map(makeVarDecl,fields),bindingsResult.value)) :
            makeFailure('Malformed bindings in "class" expression')
}
