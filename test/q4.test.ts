...
describe('Q4 Tests', () => {
     ...
     it('parse "if" expressions', () => {
         ...
         expect(l2ToPythonResult(`(if (not a) 4 5)`)).to.deep.equal(makeOk(`(4 if (not a) else 5)`));
         expect(l2ToPythonResult(`not`)).to.deep.equal(makeOk(`not`));
     });

     it('not test', ()=>{
         expect(l2ToPythonResult(`(if (not b) 5 4)`)).to.deep.equal(makeOk(`(5 if (not b) else 4)`));
     })
